...
function score (x, y, currentPlayer){
    let playerscores =[0,0]

    let player_previous_indices = [[],[]];
    if (playerscores[currentPlayer-1]==0){
        playerscores[currentPlayer-1] = 1;
    }

    else if (player_previous_indices[currentPlayer-1].length == 0){
        player_previous_indices[currentPlayer-1] =[x,y];

    } else if ((player_previous_indices[currentPlayer-1][0] || player_previous_indices[currentPlayer-1][1]) == (x) || (player_previous_indices[currentPlayer-1][0] || player_previous_indices[currentPlayer-1][1]) == (y)){
            playerscores[currentPlayer-1] += 1;
